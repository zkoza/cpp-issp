...
Napisanie funkcji, która w sposób rekurencyjny wyznaczać będzie wartość symbolu Newtona $N \choose  k$, czyli liczby $k$​-elementowych podzbiorów zbioru N-elementowego. Symbol ten posiada znaną ze szkoły definicję rekurencyjną:

$\displaystyle{{n \choose k}={\begin{cases}1&{\mbox{dla }}k=0{\mbox{ lub }}k=n\\{n-1 \choose k-1}+{n-1 \choose k}&{\mbox{dla }}0<k<n\end{cases}}}$

W tym miejscu warto przytoczyć jej uzasadnienie w wersji, która przyda się w uzasadnieniu poprawności wzoru rekurencyjnego dla problemu plecakowego. 
...
Po trzecie, jeżeli pytamy, na ile sposobów można zapalić $k$ spośród $N$ lampek, przy czym $k >0$, to wybieramy jakąś konkretną lampkę i stwierdzamy, że albo należy ona do zbioru zapalonych lampek, albo nie należy. Jeżeli należy, to problem redukuje się nam do kwestii liczby wyborów $k-1$ lampek, które muszą być wybrane do zapalenia ze zbioru $N-1$ elementowego. Jeżeli zaś nie należy, to problem redukuje się do liczby wyborów $k$ lampek w zbiorze $N-1$ elementowym. Łącznie więc mamy ${N \choose k} = {N -1 \choose k - 1} + {N -1 \choose {k}} $ , c.b.d.o.
...
Jeżeli jednak spróbujemy jej użyć np w taki prostym programie testowym wyznaczającym wartości $2k \choose k$ dla k = 1,...,17, to już mniej więcej przy k = 13 zaobserwujemy wyraźne zwolnienie programu. 
...
Drzewo to utworzyłem, po prostu usuwając powtarzające się węzły wraz z ich poddrzewami.  

Węzłów jest teraz 11. Niby niewiele mniej niż 19 w przypadku pominięcia spamiętywania, łatwo jednak wykazać, że liczba wywołań funkcji `silnia` ze spamiętywaniem nie może przekroczyć liczby elementów trójkąta Pascala w wierszach od 0 do $n-1$. Ponieważ jest ich $n(n+1)/2$, złożoność takiego algorytmu nie będzie co najwyżej kwadratowa, $O(n^2$). To jest gigantyczny postęp w porównaniu do wykładniczego $2^n$. Na przykład dla $n=100$ mamy ze spamiętywaniem nie więcej niż ok 5000 zamiast $10^{29}$ wywołań.

## Spamiętywanie (memoization)

### Wersja ze `std::map`
Najprostszy pojęciowo i najbardziej uniwersalny kod realizujący funkcję `newton` ze spamiętywaniem opiera się na mapie (`std::map`). Oczywiście zamiast funkcji posługujemy się teraz klasą, dzięki czemu możemy w jednym obiekcie umieścić zarówno funkcję, jak i stowrzyszoną z nią pamięć. 

#### Plik nagłówkowy

Plik nagłówkowy może wyglądać nastepująco:
...
Następnie sprawdzamy, czy dla danego zestawu argumentów, `n, k`,  wartość  $n \choose k$ nie została wcześniej zapisana w  obiekcie `mapa` (wyrażenie`if (it == mapa.end())`). Jeśli przypadku ustalamy wartość $n \choose k$​ na podstawie wywołania rekurencyjnego funkcji `operator()(int, int)`, a następnie umieszczamy ją w mapie. W przeciwnym wypadku odczytujemy gotową wartość z obiektu `mapa` z pomocą uzyskanego wcześniej iteratora `it`. 
...
Przedstawiona powyżej wersja, w której dane przechowujemy w kontenerze typu `std::map` jest uniwersalna, łatwa w implementacji i bezpieczna, jednak niespecjalnie efektywna, gdy z wyszukiwanie kluczy w `std::map` jest znacznie bardziej kosztowne niż np. w `std::vector`. Spróbujmy więc napisać wersję klasy `Newton`, w której dane będą przechowywane w kontenerze `std::vector`. Oto możliwa implementacja:
...
Powyższa definicja funkcji `operator()(int , int);` dość znacznie różni się od wersji opartej na `std::map`.  Przede wszystkim zakładam tu, że wartość `n` jest a priori nie znana, więc wektor `tab` początkowo jest pusty (podobni było z `std::map`). Wynika to z... braku jawnego konstruktora, co wymusza na kompilatorze zainicjowanie składowej `tab` jej konstruktorem domyślnym, ten zaś konstruuje wektor pusty. Następnie przy każdym wywołaniu funkcji `operator()(int n, int k)` i sprawdzeniu, że nie możemy od razu wyznaczyć wartości tej funkcji (gdy `k == 0 || k == n`) , musimy sprawdzić, że wektor `tab` ma co najmniej `n + 1` elementów tak, by można było go indeksować . Jeżeli ich nie ma, to zwiększamy rozmiar tablicy do `n + 1`. Ponieważ elementami tablicy `tab` są tablice liczb, musimy rozszerzyć je do pożądanego rozmiaru, pamiętając, że n-ty wiersz trójkąta Pascala ma `n + 1` elementów. Wszystkie nowe elementy inicjalizujemy  `-1` (zapisaną jako `invalid_value()`), która nie występuje w trójkącie Pascala. Za każdym razem, gdy będziemy potrzebować wartości $n \choose k$, a wartość odczytana z `tab[n][k]` będzie ujemna, stanowić to będzie dla nas sygnał, że właśnie po raz pierwszy wywołujemy funkcję ``operator()`  z tą konkretną parą argumentów `(n, k)`, dlatego musimy tę wartość wygenerować i zapisać w `tab[n, k]`.  Dzięki temu funkcja zawsze zwraca `tab[n, k]`. 

Warto jeszcze zauważyć, że aby wywołać rekurencyjnie przeciążony `operator()`, trzeba lekkiej ekwilibrystyki za składnią języka. Możemy napisać, jak powyżej, `operator()(n - 1, k - 1)`. Można by tu także użyć wyrażenia `(*this)(n - 1, k - 1)` lub nawet `this->operator()(n - 1, k - 1)`. 

Reasumując, wersja ze spamiętywaniem w kontenerze typu mapa (`std::map`) jest prostsza, gdyż łatwo sprawdzić, czy dany element jest w niej przechowywany, łatwo go z niej odczytać i łatwo do niej wstawić. Wersja z `std::vector` prawdopodobnie działać będzie szybciej, ale może potrzebować więcej pamięci, a jej implementacja wymaga pilnowania, by przypadkiem nie sięgnąć poza rozmiar tablicy.   
